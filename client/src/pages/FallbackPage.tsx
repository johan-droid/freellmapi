    ...
    <div className={`p-4 border-b last:border-0 bg-card flex flex-col gap-3 ${row.enabled ? '' : 'opacity-50'}`}>
      ...
      <div className="flex flex-wrap gap-2 text-xs">
        <div className="flex items-center gap-1.5 px-2 py-1 rounded-md bg-muted/50">
          ...
          <span><span className="text-muted-foreground">Rel:</span> {Math.round((row.reliability ?? 0) * 100)}%</span>
        </div>
        <div className="flex items-center gap-1.5 px-2 py-1 rounded-md bg-muted/50">
          <span className="size-2 rounded-sm" style={{ background: '#3b82f6' }} />
          <span><span className="text-muted-foreground">Spd:</span> {Math.round((row.speed ?? 0) * 100)}%</span>
        </div>
        <div className="flex items-center gap-1.5 px-2 py-1 rounded-md bg-muted/50">
          <span className="size-2 rounded-sm" style={{ background: '#a855f7' }} />
          <span><span className="text-muted-foreground">Int:</span> {Math.round((row.intelligence ?? 0) * 100)}%</span>
        </div>
        <div className="flex items-center gap-1.5 px-2 py-1 rounded-md bg-muted/50">
          <span>🛡️ <span className="text-muted-foreground">Grd:</span> {((row.headroom ?? 1) * (row.rateLimit ?? 1)).toFixed(2)}</span>
        </div>
      </div>
