...
function parseResetAtFromHeader(raw: string | null, now = Date.now()): string | null {
  ...
  // Reset headers vary by provider: some expose seconds remaining, some a unix
  // timestamp in seconds, and a few may already send milliseconds. Treat small
  // values as relative seconds, large values as absolute epoch timestamps.
...
export function inferQuotaPoolKey(platform: Platform, modelId?: string | null): string {
  ...
  if (platform === 'openrouter') {
    return normalizedModelId.endsWith(':free') ? 'openrouter::free' : 'openrouter::account';
  }
...
function maybeAddObservation(
  ...
  base: Omit<QuotaObservationInput, 'metric' | 'limit' | 'remaining' | 'resetAt' | 'retryAfterMs' | 'resetStrategy'>,
  metric: QuotaMetric,
...
export function parseQuotaObservationsFromResponse(
  ...
  const platform = opts.platform ?? getQuotaObservationContext()?.platform;
  if (!platform) return [];
  ...
  const base = {
    platform,
    keyId: opts.keyId ?? getQuotaObservationContext()?.keyId ?? 0,
    providerAccountId: opts.providerAccountId ?? getQuotaObservationContext()?.providerAccountId ?? null,
    modelId: opts.modelId ?? getQuotaObservationContext()?.modelId ?? null,
    quotaPoolKey: opts.quotaPoolKey ?? getQuotaObservationContext()?.quotaPoolKey ?? inferQuotaPoolKey(platform, opts.modelId ?? getQuotaObservationContext()?.modelId),
    endpoint: opts.endpoint ?? getQuotaObservationContext()?.endpoint ?? null,
  };

  const observations: QuotaObservationInput[] = [];
  const specs = HEADER_SPECS[platform];
  if (specs) {
  ...
  if (observations.length === 0 && isSharedPool(platform) && response.status === 200) {
    observations.push({
      ...base,
      metric: 'requests',
      limit: null,
...
export function recordQuotaObservation(input: QuotaObservationInput): ProviderQuotaObservation | null {
  const platform = input.platform ?? getQuotaObservationContext()?.platform;
  if (!platform) return null;

  const context = getQuotaObservationContext();
  const keyId = input.keyId ?? context?.keyId ?? 0;
  const quotaPoolKey = input.quotaPoolKey ?? context?.quotaPoolKey ?? inferQuotaPoolKey(platform, input.modelId ?? context?.modelId);
...
export function recordQuotaObservationsFromResponse(
  ...
  return parseQuotaObservationsFromResponse(response, opts)
    .map(observation => recordQuotaObservation(observation))
...
export function estimateProviderQuotaMetric(platform: Platform, modelId?: string | null): QuotaMetric {
  if (platform === 'nvidia') return 'credits';
  if (platform === 'cloudflare') return 'neurons';
  if (platform === 'huggingface') return 'credits';
  if (platform === 'pollinations' || platform === 'llm7' || platform === 'kilo') return 'requests';
  if (platform === 'google' || platform === 'openrouter' || platform === 'groq' || platform === 'cerebras' || platform === 'sambanova' || platform === 'mistral' || platform === 'github' || platform === 'cohere' || platform === 'zhipu' || platform === 'opencode') return 'requests';
  return modelId?.includes('embed') ? 'credits' : 'requests';
}
