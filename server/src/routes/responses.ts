...
import { isUnifyEnabled, getModelGroups, resolveRequestedIdToMembers, resolveRequestedIdForDispatch } from '../services/model-groups.js';
...
responsesRouter.post('/responses', async (req: Request, res: Response) => {
  ...
  if (isAutoModel(requestedModelLabel)) {
    preferredModel = resolveStickyPreference(getStickyModel(messages, sessionIdHeader, strategyKey === 'auto' ? undefined : strategyKey));
  } else {
    const db = getDb();
    ...
    if (members && members.length > 0) {
      ...
      stickyStrategyKey = requestedModelLabel;
  ...
  // Model resolution already completed above.
  ...
  await runFallbackLoop({
    ...
    route: () => routeRequest(
      ...
      groupChain ?? (strategyKey === 'auto' ? undefined : resolvedChain?.chain),
      undefined,
      completionOpts.response_format !== undefined,
      state.skipPlatforms.size > 0 ? state.skipPlatforms : undefined,
      !isAutoModel(requestedModelLabel),
    ),
    dispatch: async (route, attempt) => {
      ...
      if (stream) {
        ...
        try {
          ...
          setStickyModel(messages, route.modelDbId, sessionIdHeader, stickyStrategyKey === 'auto' ? undefined : stickyStrategyKey);
          traceRouteEvent('Responses', {
            event: 'ok',
            requestId: requestGroupId,
            attempt,
    ...
    onRoutingExhausted: (lastError, routeErr, exhaustion, info) => {
      if (streamStarted) {
        ...
        const meta = buildFallbackMeta(attemptedRoutes);
        res.status(exhaustion.status).json({ error: exhaustionErrorPayload(exhaustion), ...meta });
      }
    },
