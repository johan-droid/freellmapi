...
const updateKeySchema = z.object({
  ...
  || data.modelScope !== undefined
  || data.proxyUrl !== undefined,
{
  message: 'At least one of enabled, label, accountName, accountEmail, externalId, options, modelScope or proxyUrl must be provided',
});
...
keysRouter.post('/', (req: Request, res: Response) => {
  ...
  const result = db.prepare(`
    INSERT INTO api_keys (
      platform, label, account_name, account_email, external_id,
      encrypted_key, iv, auth_tag, status, enabled, options_json,
      proxy_encrypted, proxy_iv, proxy_auth_tag
    )
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'unknown', 1, ?, ?, ?, ?)
  `).run(
    platform, label ?? '', accountName, accountEmail, externalId,
    encrypted, iv, authTag, optionsJson,
    proxy.encrypted, proxy.iv, proxy.authTag
  );
  scheduleHydrateSecretsToRemote(db);
...
keysRouter.post('/custom', async (req: Request, res: Response) => {
  ...
  const upsert = db.transaction(() => {
    ...
    // Persist our fork's custom metadata/options to the key row!
    db.prepare(`
      UPDATE api_keys
       ...
       WHERE id = ?
    `).run(accountName, accountEmail, externalId, optionsJson, keyId);
  ...
  res.status(201).json({
    ...
    created: registered.filter((m: any) => m.created).length,
    alreadyRegistered: registered.filter((m: any) => !m.created).length,
...
keysRouter.patch('/:id', (req: Request, res: Response) => {
  ...
  const { enabled, label, accountName, accountEmail, externalId, options, modelScope, proxyUrl } = parsed.data;
