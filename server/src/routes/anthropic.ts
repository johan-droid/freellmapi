...
anthropicRouter.post('/messages', async (req: Request, res: Response) => {
  ...
  await runFallbackLoop({
    ...
    route: () => routeRequest(estimatedTotal, state.skipKeys.size > 0 ? state.skipKeys : undefined, preferredModel, hasImage, wantsTools, state.skipModels.size > 0 ? state.skipModels : undefined, undefined, undefined, false, state.skipPlatforms.size > 0 ? state.skipPlatforms : undefined, !isAutoModel(requestedModel)),
